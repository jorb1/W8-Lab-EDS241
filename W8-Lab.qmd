...
match_model <- matchit(
    proposed_turbine_3km ~ #treatment
        log_home_val_07 + p_uni_degree + log_median_inc + log_pop_denc, # covariates
    data = clean_data, 
    method = "nearest",
...
```{r}
propensity_scores <- bal.tab(propensity_scores, 
                             var.names= new_names)
...
first_stage <-
    lm(
        proposed_turbine_3km ~
            ...
            longitude + long_sq + latitude + lat_sq + long_lat +formula +
            district_id, 
        data = matched_data)
